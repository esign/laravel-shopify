 ...
 */
class VerifyCheckoutUIExtension
{
    use VerifiesSessionTokens;

    public function handle(Request $request, Closure $next)
    {
        $requestType = 'checkout-ui-extension';
        $shopDomain = null;
        ...
        try {
            ...
            $result = $shopifyApp->verifyCheckoutUIExtReq($shopifyRequest);

            if (! $result->ok) {
                throw new ShopifyAuthenticationException(
                    $requestType,
                    'Checkout UI Extension verification failed: '.($result->log->message ?? 'Unknown error')
                );
            }
            ...
            // NOTE: Checkout ID tokens are NOT exchangeable, so shop must already exist
            ...
            $this->logVerificationFailure('checkout-ui-extension', $e->getReason(), [
            ...
            $this->logVerificationFailure('checkout-ui-extension', $e->getMessage(), [
                'shop' => $shopDomain,
                'url' => $request->fullUrl(),
            ]);
