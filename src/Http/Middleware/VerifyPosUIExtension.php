 ...
 *
 * POS UI Extensions use session tokens similar to App Home and Admin UI Extensions.
 * This middleware:
 ...
 */
class VerifyPosUIExtension
{
    use VerifiesSessionTokens;

    public function handle(Request $request, Closure $next)
    {
        $requestType = 'pos-ui-extension';
        $shopDomain = null;
        ...
        try {
            ...
            $result = $shopifyApp->verifyPosUIExtReq($shopifyRequest);

            if (! $result->ok) {
                throw new ShopifyAuthenticationException(
                    $requestType,
                    'POS UI Extension verification failed: '.($result->log->message ?? 'Unknown error')
                );
            }
            ...
            $this->logVerificationFailure('pos-ui-extension', $e->getReason(), [
            ...
            $this->logVerificationFailure('pos-ui-extension', $e->getMessage(), [
                'shop' => $shopDomain,
                'url' => $request->fullUrl(),
            ]);
